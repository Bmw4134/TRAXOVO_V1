...
class GeniusPerformanceEngine {
  ...
  async processQueue() {
    if (
      this.activeRequests >= GENIUS_CONFIG.maxConcurrentRequests ||
  ...
  setupIntelligentCaching() {
    ...
    this.getCached = (key) => {
      ...
      if (
        cached &&
        Date.now() - cached.timestamp < GENIUS_CONFIG.cacheTimeout
      ) {
        return cached.data;
      }
      return null;
  ...
  prefetchCriticalResources() {
    const criticalAPIs = ["/api/fleet/assets", "/api/performance/metrics"];

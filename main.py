...
def pm_allocation_processor():
    ...
    import traceback
    import time
    from utils.pm_processor import process_pm_allocation_files
    from werkzeug.utils import secure_filename
    ...
    if request.method == 'POST':
        ...
        if not (allowed_file(original_file.filename) and allowed_file(updated_file.filename)):
            flash('Only Excel files (.xlsx, .xlsm, .xls) are allowed', 'danger')
            return redirect(request.url)
        ...
        try:
            ...
            original_path = uploads_dir / secure_filename(original_file.filename)
            updated_path = uploads_dir / secure_filename(updated_file.filename)
            
            original_file.save(original_path)
            updated_file.save(updated_path)
            
            # Process the files and get results
            results = process_pm_allocation_files(original_path, updated_path, region)
            
            if 'error' in results:
                flash(f"Error processing PM allocation files: {results['error']}", 'danger')
                return redirect(request.url)
            
            # Format currency values for display
            if not hasattr(app.jinja_env, 'filters'):
            ...
            return render_template('pm_allocation.html', 
                                  title='PM Billing Allocation Processor',
                                  results=results)
    ...
    # Find recent jobs if any
    recent_jobs = []
    exports_dir = Path('exports')
    if exports_dir.exists():
        excel_files = list(exports_dir.glob('pm_allocation_reconciliation_*.xlsx'))
        excel_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
        
        for i, file_path in enumerate(excel_files[:3]):  # Show 3 most recent
            timestamp = file_path.stem.replace('pm_allocation_reconciliation_', '')
            date_obj = datetime.strptime(timestamp, '%Y%m%d_%H%M%S')
            
            recent_jobs.append({
                'date': date_obj.strftime('%m/%d/%Y %H:%M'),
                'file': file_path.name,
                'region': 'All Regions',  # We'd need to parse the file to get the actual region
                'changes': '---',  # We'd need to parse the file to get the actual count
                'view_url': url_for('view_pm_report', filename=file_path.name),
                'download_url': url_for('download_pm_report', filename=file_path.name)
            })
...
@app.route('/auto_process_pm_allocation')
@login_required
def auto_process_pm_allocation():
    """Automatically find and process PM allocation files"""
    from utils.pm_processor import auto_detect_and_process_allocation_files
    
    # Process the allocation files
    results = auto_detect_and_process_allocation_files()
    ...
    flash("Successfully processed PM allocation files automatically", "success")
    return render_template('pm_allocation.html', 
                          title='PM Billing Allocation Processor',
                          results=results)
...
def view_pm_report(filename):
    ...
    # For Excel files, we would need to extract data
    # For now, just redirect to download
    return redirect(url_for('download_pm_report', filename=filename))
...
def auto_process_pm_allocation():
    ...
    # Check for allocation files in the attached_assets directory

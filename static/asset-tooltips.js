// TRAXOVO Asset Tooltip System - Authentic GAUGE Data Integration
class AssetTooltipManager {
  ...
  async loadTooltipData() {
    try {
      ...
      if (data.success && data.assets) {
        ...
        console.log(
          `Loaded tooltips for ${Object.keys(this.tooltipData).length} authentic assets`,
        );
      }
    } catch (error) {
    ...
    document.addEventListener(
      ...
      (e) => {
        const assetElement =
          ...
          e.target.parentElement.querySelector("[data-asset-id]");
        if (assetElement) {
  ...
  generateTooltipHTML(tooltipInfo) {
    const statusClass =
      tooltipInfo.status === "Active" ? "status-active" : "status-inactive";

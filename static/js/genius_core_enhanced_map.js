...
class EnhancedMapSystem {
    constructor() {
        ...
        // Initialize map enhancements after a delay to ensure the map is loaded
    ...
    createCustomPopupTemplate() {
        // Create a popup template function that will be used for all markers
        this.popupTemplate = (asset) => {
            ...
            // Add job scoring section if location is known
            if (asset.location) {
                let jobNumber = null;
                ...
                // If job number found, show score
                if (jobNumber) {
                ...
                // Create mark as billable button
    ...
    setupMarkerListeners() {
        // Since the markers are created dynamically, we need to set up a MutationObserver
        // to detect when new markers are added to the map
        
        const mapContainer = document.getElementById('map');
        if (!mapContainer) return;
        
        // Set up observer to watch for added markers
    ...
    handleMarkerClick(assetId) {
        // This method would be called when a marker is clicked
        // In a real implementation, it would get the asset data and show a custom popup
        
        // Log the click
        console.log('Marker clicked:', assetId);
        
        // In the actual implementation, the popup would be created by Leaflet
        // and we'd just be enhancing it. For now, we'll simulate this for demo.
    }
    
    enhanceJobSiteVisualization() {
        // Enhance job site shapes on the map
        // In a real implementation, this would add visual indicators to job sites
    ...
    registerMapData() {
        // Register map data with the continuity system
    ...
    updateJobScoreDisplay(container, assetId, jobNumber) {
        // Get job score from telemetry or confidence system
        let score = this.getJobScore(assetId, jobNumber);
        
        // If we got a score, display it
        if (score) {
    ...
    getJobScore(assetId, jobNumber) {
        // In a real implementation, this would get the actual score
        // from the telemetry or confidence system
        
        // For demonstration, we'll return a mock score if VisualDiagnostics is available
        ...
        // Otherwise return a default score
        return {
            assetId: assetId,
            jobNumber: jobNumber,
            overallScore: 75,
    ...
    showConfirmation(button, message) {
        // Create or get confirmation message element
    ...
    assignAssetToJob(assetId, jobNumber, driverId = null) {
        ...
        // Update continuity
        if (window.ContinuityManager) {
            // In a real implementation, this would update the asset data
            // and trigger cross-validation with other modules
    ...
    markAssetAsBillable(assetId, jobNumber, isPMBillable = true) {
        ...
        // Update billing module if available
    ...
    flagAssetForReview(assetId, reason, priority = 'medium') {
        ...
        // Register conflict if visual diagnostics is available
        if (window.VisualDiagnostics) {
    ...
    highlightAsset(assetId, style = 'default') {
        // This would highlight an asset on the map
        // In a real implementation, this would change the marker style

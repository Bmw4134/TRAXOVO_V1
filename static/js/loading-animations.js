...
function showLoader(containerId, type = 'excavator', message = 'Loading...', fadeIn = true) {
    ...
    switch (type) {
        ...
        case 'hardhat-spinner':
            ...
            break;
        default:
            // Default to excavator if invalid type
            loader = createExcavatorLoader();
            break;
    }
    ...
    // Add loading dots animation to message
    if (message.endsWith('...')) {
...
function initializeLoadingButtons() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', function(e) {
            // Don't add loading state if button is disabled
            if (this.disabled) {
                return;
            }
            ...
            // Cancel default action if form has validation errors
            if (targetForm) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add global AJAX event handlers for loading state
    if (typeof $ !== 'undefined') {

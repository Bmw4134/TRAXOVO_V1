...
function initializeMap(assets) {
  ...
  map = L.map("map-container").setView([32.7767, -96.797], 10); // Dallas area
...
function filterMapMarkers(status, category, location) {
  fetch(
    `/api/assets?status=${status}&category=${category}&location=${location}`,
  )
    .then((response) => response.json())
    .then((data) => {
      if (map) {
    ...
    .catch((error) =>
      console.error("Error fetching filtered asset data:", error),
    );
}

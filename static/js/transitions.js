...
function initPageTransitions() {
  const contentSections = document.querySelectorAll(
    ".page-transition-container > div, .page-transition-container > section",
  );
  contentSections.forEach((section) => {
    if (
      !section.classList.contains("fade-transition") &&
  ...
  const pageLinks = document.querySelectorAll(
    'a:not([target="_blank"]):not([href^="#"]):not([href^="javascript"])',
  );
  pageLinks.forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      if (
        ...
        document.querySelector(".page-transition-container").style.transition =
          "opacity 0.3s ease";
        setTimeout(() => {
...
function initCardAnimations() {
  ...
  cards.forEach((card) => {
    if (
      !(
        card.parentElement &&
        card.parentElement.classList.contains("no-animation")
      ) &&

...
class TRAXOVOPDFExporter {
    ...
    addExportButtons() {
        // Add export buttons to various dashboards
        const dashboards = [
            { selector: '.dashboard-container', title: 'Executive Dashboard' },
            { selector: '.equipment-billing-dashboard', title: 'Equipment Billing Report' },
        ...
        // Add main export button to header/navbar if exists
    ...
    async exportToPDF(container, reportTitle) {
        if (typeof jsPDF === 'undefined') {
            this.showExportError('PDF library not loaded. Please refresh and try again.');
            return;
        }
    ...
    async exportDashboard() {
        // Collect authentic dashboard data
    ...
    async exportBillingReport() {
        ...
        try {
            ...
            if (billingData.equipment_breakdown) {
                yPos = this.addSectionTitle(pdf, 'Equipment Category Breakdown', yPos + 15);
                yPos = this.addEquipmentBreakdown(pdf, billingData.equipment_breakdown, yPos);
            }
    ...
    addPDFFooter(pdf) {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            pdf.text(`Report generated on ${new Date().toLocaleDateString()}`, 150, 295);
        }
    }
    ...
    async collectDashboardData() {
        // Collect authentic data from various endpoints
        const data = {
            metrics: {},
            revenue: {},
            utilization: {}

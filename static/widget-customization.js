    ...
    class WidgetCustomizer {
        constructor() {
            ...
            this.availableWidgets = [
                ...
                { id: 'revenue-metrics', name: 'Revenue Metrics', category: 'financial' },
                { id: 'asset-utilization', name: 'Asset Utilization', category: 'analytics' },
                { id: 'driver-performance', name: 'Driver Performance', category: 'operations' },
                { id: 'maintenance-alerts', name: 'Maintenance Alerts', category: 'operations' },
                { id: 'billing-summary', name: 'Billing Summary', category: 'financial' },
                { id: 'efficiency-trends', name: 'Efficiency Trends', category: 'analytics' },
                { id: 'weather-overview', name: 'Weather Overview', category: 'external' }
            ];
        ...
        saveUserPreferences() {
            localStorage.setItem('traxovo_widget_preferences', JSON.stringify(this.userPreferences));
        ...
        showCustomizationPanel() {
            ...
            // Set current values
            document.querySelector(`[data-layout="${this.userPreferences.layout}"]`)?.classList.add('selected');
            document.getElementById('auto-refresh-toggle').checked = this.userPreferences.autoRefresh;
            document.getElementById('refresh-interval').value = this.userPreferences.refreshInterval;
        }
        ...
        applyLayout() {
            const dashboardContainer = document.querySelector('.main-content') || document.querySelector('.dashboard-container');
        ...
        applyWidgetVisibility() {
            this.availableWidgets.forEach(widget => {
                ...
                if (element) {
                    element.style.display = this.userPreferences.enabledWidgets.includes(widget.id) ? 'block' : 'none';
                }
            });
        }
        ...
        applyChanges() {
            ...
            document.querySelectorAll('[data-widget-id]:checked').forEach(checkbox => {
                enabledWidgets.push(checkbox.dataset.widgetId);
            });
            this.userPreferences.enabledWidgets = enabledWidgets;
            
            // Get settings
            this.userPreferences.autoRefresh = document.getElementById('auto-refresh-toggle').checked;
            this.userPreferences.refreshInterval = parseInt(document.getElementById('refresh-interval').value);

...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const reportPreviewsContainer = document.getElementById(
    "report-previews-container",
  );
  const noReportsMessage = document.getElementById("no-reports-message");
  const previewModal = new bootstrap.Modal(
    document.getElementById("report-preview-modal"),
  );
  ...
  const reportDescriptions = {
    daily_driver:
      "Daily driver activity report showing late starts, early ends, and not-on-job incidents.",
    pm_allocation:
      "Preventive maintenance allocation report showing original and updated values with reconciliation.",
    asset_status:
      "Current status of all assets including location, driver, and last update time.",
    maintenance:
      "Maintenance schedule and history for equipment with upcoming service dates.",
    job_zone_efficiency:
      "Job zone efficiency report showing equipment time in zone vs expected time.",
    attendance_metrics:
      "Attendance metrics report showing trends, patterns, and flags.",
  };
  const reportFilters = {
    daily_driver: [
      {
  ...
  function handleReportTypeChange() {
    ...
    reportDescription.querySelector(".description-text").textContent =
      reportDescriptions[selectedType] || "No description available.";
  ...
  function updateFilterOptions(reportType) {
    ...
    if (filters.length === 0) {
      ...
      noFiltersDiv.innerHTML =
        "<small>No filters available for this report type</small>";
  ...
  function generateReport() {
    ...
    setTimeout(() => {
      ...
      showFeedback(
        "Success",
        `Report generated successfully! You can download it now.`,
        "success",
      );
    }, 2000);
  }
  function getReportOptions() {
    ...
    document
      .querySelectorAll('#filter-options input[type="radio"]:checked')
      .forEach((radio) => {
        filters[radio.name] = radio.value;
      });
    document
      .querySelectorAll('#filter-options input[type="range"]')
      .forEach((range) => {

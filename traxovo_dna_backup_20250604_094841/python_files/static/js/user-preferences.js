...
class UserPreferences {
  ...
  saveFieldHistory() {
    localStorage.setItem(
      "traxora_field_history",
      JSON.stringify(this.fieldHistory),
    );
  }
      ...
      if (filterElement.tagName === "SELECT") {
        ...
        if (filterElement.type === "checkbox") {
          ...
          const radio = document.querySelector(
            `input[name="${filterElement.name}"][value="${savedValue}"]`,
          );
  ...
  applyColumnPreferences() {
    ...
    Object.keys(pageColumns).forEach((columnId) => {
      ...
      const columnToggle = document.querySelector(
        `[data-column-toggle="${columnId}"]`,
      );
      if (columnToggle) {
        ...
        document
          .querySelectorAll(`[data-column="${columnId}"], .${columnId}-column`)
          .forEach((col) => {
  ...
  applySortPreferences() {
    ...
    Object.keys(pageSorts).forEach((tableId) => {
      ...
      const sortSelect = document.querySelector(
        `#${tableId}-sort-field, [data-sort-table="${tableId}"]`,
      );
      const directionSelect = document.querySelector(
        `#${tableId}-sort-direction, [data-sort-direction="${tableId}"]`,
      );
  ...
  setupFieldAutoComplete() {
    document
      ...
      .forEach((field) => {
        if (
          field.classList.contains("no-autocomplete") ||
        ...
        if (
          ...
          this.fieldHistory[fieldId].length > 0
        ) {
  ...
  initializeFormTracking() {
    document
      .querySelectorAll('.data-filter, [data-filter="true"]')
      .forEach((filter) => {
    ...
    document.querySelectorAll("[data-column-toggle]").forEach((toggle) => {
      ...
      toggle.addEventListener("change", () => {
        const isVisible =
          toggle.type === "checkbox" ? toggle.checked : toggle.value === "true";
    ...
    document
      .querySelectorAll('.search-form, [data-search="true"]')
      .forEach((searchForm) => {
  ...
  saveSearchQuery(query) {
    ...
    this.preferences.recentSearches[this.currentPage] =
      this.preferences.recentSearches[this.currentPage].filter(
        (q) => q !== query,
      );
  ...
  saveFieldInputHistory(fieldId, value) {
    ...
    this.fieldHistory[fieldId] = this.fieldHistory[fieldId].filter(
      (v) => v !== value,
    );

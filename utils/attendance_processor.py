...
def integrate_attendance_data(csv_path, payroll_data):
    """Integrate and clean attendance data from CSV and payroll sources."""
    # Load CSV data
    attendance_data = pd.read_csv(csv_path)

    # Merge with payroll data
    merged_data = pd.merge(attendance_data, payroll_data, on='employee_id', how='inner')

    # Remove duplicates
    clean_data = merged_data.drop_duplicates()

    # Prepare report format
    export_ready_data = clean_data[['employee_id', 'name', 'job_zone', 'regular_hours', 'overtime_hours']]
